...
For remote `test-plan` / one-shot runs that upload a standalone Jepsen jar as `JAR_URL`, validate that jar before upload with `bash scripts/check_jar_compat.sh /path/to/jepsen-standalone.jar`.
