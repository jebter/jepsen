...
usage() {
  cat <<'EOF'
usage: scripts/check_jar_compat.sh [--max-major <n>] [--forbid-symbol <symbol>]... <jar>
...
done < <(find "$TMP_DIR" -type f -name '*.class' -print0 | sort -z)

if [[ "$CLASS_COUNT" -eq 0 ]]; then
  echo "no class files found in jar: $JAR_PATH" >&2
  exit 1
fi

if [[ ${#MAJOR_FAILURES[@]} -gt 0 || ${#SYMBOL_FAILURES[@]} -gt 0 ]]; then
  echo "jar compatibility check failed: $JAR_PATH" >&2
...
echo "jar compatibility check passed: $JAR_PATH"
